...
class EtudiantDAO
{
    ...
    public function ConnexionEtudiant($login, $password) {
        $query = "SELECT * FROM Utilisateur WHERE logUti = :login AND mdpUti = :password AND idTypUser = 1";
        $stmt = $this->pdo->prepare($query);
        $stmt->bindParam(':login', $login);
        $stmt->bindParam(':password', $password);
        $stmt->execute();
        return $stmt->rowCount() > 0; // Retourne true si un administrateur est trouvé.
    }
